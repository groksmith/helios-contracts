...
async function main() {
    ...
    const txWithdrawFunds = await poolContract.connect(user).withdrawFunds();
    await txWithdrawFunds.wait(1);

    const amount = 10 * 10 ** decimals;
    const txWithdraw = await poolContract.connect(user).withdraw(amount);
